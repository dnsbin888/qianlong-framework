...
架构 (双道并行, 非二选一):
  ...
  两条道同时跑: 快速通道负责执行, 审核通道负责记录

用法:
  # QMT 策略编辑器中:
  import sys
  sys.path.insert(0, r"D:\quant_framework")
  from strategies.qmt_engine import on_bar, load_pool, reset_daily, get_status
"""
...
def on_bar(symbol, bar_data, prev_bar_data, account_id="", total_asset=100000):
    ...
    if stock.get("enabled", False):
        ...
        tag = "[快速通道]" if ok else "[快速通道] ❌"
        print(f"{tag} {symbol} {detail}" if not ok else f"{tag} {detail}")
