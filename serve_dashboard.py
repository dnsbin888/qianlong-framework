"""启动 Flask 服务器，托管 dashboard.html（含 iframe 嵌入 Streamlit）。

用法:
    python serve_dashboard.py
    浏览器访问: http://localhost:5000/dashboard
...
# ---- Dashboard 页面（iframe 嵌入 Streamlit）──────────────────
@app.route('/dashboard')
def dashboard():
...
function resizeIframe(){
  var frame=document.getElementById('streamlitFrame');
  if(frame) frame.style.height=(window.innerHeight-44)+'px';
}
...
/* ---- 隐藏 Streamlit 原生顶栏（同源时生效）---- */
function hideStreamlitHeader(){
  var frame=document.getElementById('streamlitFrame');
  if(!frame || !frame.contentDocument) return;
  try{
    var h=frame.contentDocument.querySelector('header[data-testid="stHeader"]');
    if(h) h.style.display='none';
    var d=frame.contentDocument.querySelector('[data-testid="stDecoration"]');
    if(d) d.style.display='none';
  }catch(e){}
}
var frame=document.getElementById('streamlitFrame');
if(frame) frame.addEventListener('load', hideStreamlitHeader);
</script>
</body>
</html>"""
...
if __name__ == '__main__':
    PORT = 5000
    print(f'🐉 潜龙 — 策略回测页面')
    print(f'   本地访问: http://localhost:{PORT}/dashboard')
    print(f'   请确保 Streamlit 已在 localhost:8501 运行')
    print()
