...
def sim(guard_pct):
    """guard_pct: price% threshold to enter ZT-protection mode"""
    ts = []; zt_cap = 0
    for e in entries:
        ...
        for j in range(idx+1, min(idx+30, len(s['d']))):
            ...
            # Enter ZT-protection mode if peak reaches guard_pct
            if not guarded and pp >= guard_pct:
                guarded = True
            ...
            if lh and not is_zt:
                ...
                if pp >= 0.02 and (p-peak)/ep <= -0.02:
                    ts.append({"pnl": (pnl-0.0013)*(remain/100), "days": dc, "r": "ZT2"})
                    break

            r, sp = None, 0

            # TP logic depends on guard status
            if guarded:
                # Protected mode: wider stop, wait for ZT or big pullback
                if not h7:
                    h7 = True  # Skip TP1 entirely
                if (p-peak)/ep <= -0.03:
                    r, sp = "T2g", remain  # Sell only on 3% pullback
            else:
                # Normal mode: tight stops
                if not h7 and pp >= 0.05 and (p-peak)/ep <= -0.010:
                    r, sp = "T1", 50; h7 = True
            ...
            if r is None:
                ...
                elif dc >= 5 and pnl < 0.01: r, sp = "Tm", remain
    ...
    return (len(ts), wr, aw, al, pf, total, zt_cap)

THRESHOLDS = [0.065, 0.070, 0.075, 0.080, 0.085, 0.090]
print(f"Entries: {len(entries)}\n")
print(f"  {'Guard%':<8} {'Trd':>5} {'WR':>6} {'PF':>6} {'PnL':>10} {'ZTcap':>6}")
for g in THRESHOLDS:
    nt, wr, aw, al, pf, pnl, zt = sim(g)
    print(f"  {g*100:>5.1f}%  {nt:>5} {wr:>5.1%} {pf:>6.2f} {pnl:>+10.2f} {zt:>6}")

# Baseline (no guard)
nt, wr, aw, al, pf, pnl, zt = sim(0.0)
print(f"  no guard {nt:>5} {wr:>5.1%} {pf:>6.2f} {pnl:>+10.2f} {zt:>6}")
