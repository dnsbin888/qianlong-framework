...
def _handlebar_impl(ContextInfo):
    ...
    if _first_bar:
        print(f"[潜龙] ✅ handlebar触发! 标的={len(ContextInfo.trade_code_list)}只")
        _first_bar = False
    ...
    for qmt_code in ContextInfo.trade_code_list:
        ...
        # 打板·一封 (广度刀): bar.high预筛 + 实时价触板 + 重过滤在评分段
        if not signal_name:
            _lim = round(prev_close * (1 + _lp), 2)
            if bar_high >= _lim * 0.995 and prev_vol > 0 and volume > prev_vol * 1.5:
                # 实时价: 不等bar收盘, get_full_tick取此刻lastPrice
                try:
                    _ft = ContextInfo.get_full_tick(qmt_code)
                    _rt = float(_ft.lastPrice) if hasattr(_ft, 'lastPrice') else price
                except:
                    _rt = price
                if _rt >= _lim * 0.995:
                    if time.strftime('%H%M') < '1400':                     # 游资: 午后不追
                        if _sentiment_stage != "retreat":                   # 私募: 退潮不做
                            if _advance_ratio >= 0.5 and _limit_down_count <= 50:  # 私募: 涨多跌少
                                signal_name = "打板·一封"
