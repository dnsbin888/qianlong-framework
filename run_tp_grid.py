...
def sim(tp1_drop, tp2_drop):
    ...
    for e in entries:
        ...
        for j in range(idx+1, min(idx+30, len(s['d']))):
            ...
            elif not zt and pp >= 0.02 and (p-peak)/ep <= -0.03: r, sp = "回落", remain
            elif dc >= 5 and pnl < 0.01: r, sp = "时间", remain
            if r and sp > 0:
                s2 = min(sp, remain)
...
GRID = list(itertools.product([0.010, 0.015, 0.020, 0.025, 0.030],
                                [0.010, 0.015, 0.020, 0.025, 0.030]))

print(f"Entries: {len(entries)} | Testing {len(GRID)} combos\n")
print(f"  {'+5%回':<8} {'+7%回':<8} {'Trd':>6} {'WR':>6} {'PF':>6} {'PnL':>10}")
...
for d1, d2 in GRID:
    ...
    if nt > 0:
        results.append((d1, d2, nt, wr, aw, al, pf, pnl))
        print(f"  {d1*100:>4.1f}%    {d2*100:>4.1f}%    {nt:>6} {wr:>5.1%} {pf:>6.2f} {pnl:>+10.2f}")

results.sort(key=lambda x: (-x[6], -x[3]))
print(f"\n{'='*55}")
print(f"  TOP 5 by PF:")
for rank, r in enumerate(results[:5]):
    print(f"  {rank+1}. +5%回{r[0]*100:.1f}%/+7%回{r[1]*100:.1f}%  PF={r[6]:.2f} WR={r[3]:.1%} PnL={r[7]:+.2f}")

results.sort(key=lambda x: (-x[3], -x[6]))
print(f"\n  TOP 5 by WR:")
for rank, r in enumerate(results[:5]):
    print(f"  {rank+1}. +5%回{r[0]*100:.1f}%/+7%回{r[1]*100:.1f}%  WR={r[3]:.1%} PF={r[6]:.2f} PnL={r[7]:+.2f}")
