...
CONFIG = {
    ...
    "max_single_position_pct": 20,     # E372: 单票最大仓位%(对齐trade_config)
    "max_sector_pct": 25,              # E372: 同行业最大仓位%
    "max_daily_trades": 99,            # E372: 不再设笔数限制(用仓位+熔断替代)
    "max_daily_loss": -5.0,            # 日内最大亏损%
    "max_consecutive_loss": 99,        # E372: 不再限制(用日亏损熔断替代)
    "max_hold_days": 7,                # E372: 持仓7天(对齐私募)
    "max_cancel_rate": 0.50,           # 撤单率>50%触发风控
    "max_order_value": 1_000_000,      # 单笔最大金额(元)
    "auto_trade_enabled": False,
    "signal_min_strength": 3,          # E372: 统一=3
    "trading_channel": "ths",  # "ths"=同花顺联动精灵, "qmt"=QMT XtQuant
    "real_confirmed": False,       # 🔒 实盘双锁: 必须老板手动设为true才执行实盘
}
...
if os.path.exists(_cfg_file):
    try:
        ...
        if isinstance(_data, dict):
            ...
            # 从master加载基线参数 (统一真相源)
            try:
                from config_loader import load_tp_params, load_risk_params
                CONFIG.update(load_tp_params())
                CONFIG.update(load_risk_params(channel="real"))
                print(f"[Trader] Master参数已加载: TP3_sell={CONFIG.get('tp3_sell_ratio')}, "
                      f"硬止损={CONFIG.get('hard_stop_loss')}, max_pos={CONFIG.get('max_positions')}")
            except Exception as _e:
                print(f"[Trader] Master加载失败(使用旧默认): {_e}")
            CONFIG.update(_data)
            # 风控总闸: AI自动开关 → 覆盖本地的 auto_trade_enabled
            try:
                _plan_p = r"D:\quant_web\data\auto_trade_plan.json"
                if os.path.exists(_plan_p):
                    with open(_plan_p, "r", encoding="utf-8") as _pf:
                        _ai = json.load(_pf).get("global_limits", {}).get("ai_auto_enabled")
                    if _ai is not None:
                        CONFIG["auto_trade_enabled"] = bool(_ai)
                        print(f"[Trader] KS ai_auto_enabled={_ai} → auto_trade_enabled 已同步")
...
try:
    ...
    if _os6.path.exists(sp):
        ...
        # 从master读取策略默认值
        try:
            from config_loader import get_param as _gp
            _def_tp = [_gp("tp1_profit_pct"), _gp("tp2_profit_pct"), _gp("tp3_profit_pct")]
            _def_sl = _gp("soft_stop_loss")
            _def_hd = _gp("max_hold_days")
        except Exception:
            _def_tp, _def_sl, _def_hd = [0.05, 0.07, 0.10], -0.03, 3
        for s in strategies:
            if s.get("status") in ("real", "sim", "sim_running") and s.get("type") == "builder":
                tp = s.get("take_profit", _def_tp)
                STRATEGY_PARAMS[s["name"]] = {
                    "stop_loss": s.get("stop_loss", _def_sl),
                    "take_profit": tp,
                    "hold_days": s.get("hold_days", _def_hd),
...
class TradeState:
    ...
    def to_dict(self):
        return {
            ...
            "pnl": self.pnl,
            "risk": self.risk,
            "auto_trade_enabled": _get_auto_status(),
...
class KeyboardTrader:
    ...
    def send_buy_order(self, code=None, price=None, quantity=None):
        ...
        try:
            ...
            _result = _checker.check_buy(clean_code, p, q, signal_level=3, live_mode=True)
            if len(_result) == 2: _result = (_result[0], _result[1], q)  # 兼容旧版
            _action, _reason, _adj_qty = _result
            if _action is False or _action == "REJECT":
...
class AutoTradeEngine:
    def __init__(self):
        ...
        self.position_peaks = {}
        # P0-4: 崩溃恢复状态文件路径
        self.STATE_FILE = r"d:\quant_framework\live_engine_state.json"
        # P0-4: 崩溃恢复 — 从磁盘恢复日内状态
        self._load_state()

    def _save_state(self):
        """P0-4: 持久化日内交易状态, 防崩溃重启后风控绕过"""
        try:
            import os as _os
            data = {
                ...
                "position_peaks": self.position_peaks,
                "updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            }
            tmp = self.STATE_FILE + ".tmp"
            with open(tmp, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False)
            _os.replace(tmp, self.STATE_FILE)  # 原子写入
        except Exception as e:
            print(f"[AutoTrade] 状态保存失败: {e}")

    def _load_state(self):
        """P0-4: 从磁盘恢复日内状态 (同日有效, 跨日归零)"""
        try:
            import os as _os
            ...
            with open(self.STATE_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            saved_date = data.get("last_trade_date")
            today = str(datetime.now().date())
            if saved_date == today:
                self.daily_trade_count = data.get("daily_trade_count", 0)
                self.consecutive_losses = data.get("consecutive_losses", 0)
                self.position_peaks = data.get("position_peaks", {})
                if self.daily_trade_count or self.consecutive_losses:
                    print(f"[AutoTrade] 状态恢复: trades={self.daily_trade_count}, "
                          f"cons_loss={self.consecutive_losses}, peaks={len(self.position_peaks)}")
            else:
                print(f"[AutoTrade] 状态过期({saved_date}), 新交易日从零开始")
        except Exception as e:
            print(f"[AutoTrade] 状态加载失败: {e}")
    ...
    def check_rules(self, signal_data=None):
        ...
        if self.last_trade_date != now.date():
            ...
            self._save_state()  # P0-4: 新交易日立即持久化
        ...
        if signal_data and CONFIG["auto_trade_enabled"] and self.daily_trade_count < CONFIG["max_daily_trades"]:
            ...
            try:
                ...
                from master_switch import can_buy as _ms_can_buy, read_plan as _ms_read_plan
                if not _ms_can_buy("real"):
                    print("[AutoTrade] ⛔ 风控总闸关闭, 跳过买入")
                    signal_data = None
                elif not _ms_read_plan().get("ai_auto_enabled", False):
                    print("[AutoTrade] ⛔ AI自动交易已关闭, 跳过买入")
                    signal_data = None
            except Exception as _ms_e:
                print(f"[AutoTrade] 总闸检查异常(放行): {_ms_e}")
            for sig in (signal_data or [])[:10]:
                ...
                with _BUY_COOLDOWN_LOCK:  # E01: 防竞态
                    _nsym = _normalize_sym(sym)
                    if _nsym in _BUY_COOLDOWN:
                        if time.time() - _BUY_COOLDOWN[_nsym] < 1800:
                            print(f"[AutoTrade] 跳过{sym}: 30分钟冷却中")
                            continue
                        else: del _BUY_COOLDOWN[_nsym]
                # E266: 黑名单检查
                try:
                    from blacklist import is_blocked
                    if is_blocked(sym): continue
                except Exception as _bl_e:
                    print(f"[Trader] ❌ 黑名单模块加载失败: {_bl_e} — 安全侧: 拦截所有交易")
                    is_blocked = lambda s: True  # 兜底: 全部拦截, 宁可错过不冒险
                ...
                # 实时价格替代信号close: 实盘买入用当前市价, 非日线收盘价
                try:
                    from realtime_quotes import _quote_cache
                    _qc_code = sym[2:] + ('.SH' if sym.startswith('sh') else '.SZ' if sym.startswith('sz') else '.BJ')
                    _qc_rt = _quote_cache.get("data", {}).get(_qc_code, {})
                    _qc_price = _qc_rt.get("price", 0)
                    if _qc_price > 0:
                        price = _qc_price
                except Exception:
                    pass
                ...
                try:
                    ...
                    _total_eq = CONFIG.get("live_total_asset", 0) or sum(
                        ...
                        for pp in state.positions) + _cfg_cash
                    ...
                    _result = _checker.check_buy(sym, price, 100, _industry, bs, live_mode=True)
                    if len(_result) == 2: _result = (_result[0], _result[1], 100)
                    _action, _reason, _adj_qty = _result
                    if _action is False or _action == "REJECT":
    ...
    def execute_actions(self, actions):
        ...
        """
        # 🔒 实盘双锁: trading_channel=qmt + real_confirmed=true 缺一不可
        if CONFIG.get("trading_channel") == "qmt" and not CONFIG.get("real_confirmed", False):
            print("[AutoTrade] 🔒 实盘未确认(real_confirmed=false), 拒绝下单 — 请在 live_trader_config.json 设 real_confirmed=true")
            return []
        ...
        for act in actions:
            ...
            if act["type"] == "buy":
                global _BUY_COOLDOWN, _BUY_COOLDOWN_LOCK
                with _BUY_COOLDOWN_LOCK:  # E01: 防竞态
                    _BUY_COOLDOWN[_normalize_sym(act["symbol"])] = time.time()
                # A04: 成交确认
                sym = act.get("symbol","")
                def _confirm():
                    time.sleep(30)
...
def _normalize_sym(symbol: str) -> str:
    """E349: 代码格式归一 → 统一用 sh/sz+6位 (防cooldown格式不一致)"""
    import re
    s = str(symbol).strip().lower().replace('.sh','').replace('.sz','').replace('.bj','')
    s = re.sub(r'^(sh|sz|bj)', '', s)
    if s.startswith(('6','5','9')): return 'sh' + s
    elif s.startswith(('0','3','2','1')): return 'sz' + s
    elif s.startswith(('8','4')): return 'bj' + s
    return symbol  # 兜底
...
def start_auto_sync():
    ...
    tracked = _load_position_tracker()
    if tracked:
        print(f"[Trader] 持仓跟踪已加载: {len(tracked)}只")
    # E349: 启动时比对QMT实际持仓(只告警,不自动同步)
    try:
        _real_positions = trader.get_positions() if hasattr(trader, 'get_positions') else []
        if _real_positions:
            _real_syms = {p.get('symbol','') or p.get('stock_code','') for p in _real_positions}
            _tracked_syms = {t.get('symbol','') for t in tracked} if tracked else set()
            _missing = _real_syms - _tracked_syms
            _extra = _tracked_syms - _real_syms
            if _missing:
                print(f"[Trader] ⚠️ QMT有但跟踪文件无: {_missing} → 可能是手动下单或分红送股")
            if _extra:
                print(f"[Trader] ⚠️ 跟踪文件有但QMT无: {_extra} → 可能已卖出但跟踪未更新")
            if not _missing and not _extra:
                print(f"[Trader] ✅ 持仓核对一致 ({len(_real_syms)}只)")
        else:
            print(f"[Trader] QMT无持仓数据(可能未连接)")
    except Exception as _re:
        print(f"[Trader] 持仓比对跳过: {_re}")
    ...
    def _sync_loop_once():
        ...
        # E205+E349: 日亏损 = (当前总权益 - 日初权益) / 日初权益 → 包含已实现
        try:
            _day_start = getattr(state, 'day_start_equity', None)
            if not _day_start or _day_start <= 0:
                _day_start = CONFIG.get("live_total_asset", 0) or sum(
                    p.get("cost_price", 0) * p.get("quantity", 0)
                    for p in state.positions)
                if _day_start > 0:
                    state.day_start_equity = _day_start
            if not _day_start or _day_start <= 0:
                state.risk["daily_loss"] = 0
            else:
                _current_eq = CONFIG.get("live_total_asset", 0) or sum(
                    p.get("market_value", 0) + p.get("cost_price", 0) * p.get("quantity", 0)
                    for p in state.positions)
                state.risk["daily_loss"] = round(
                    (_current_eq - _day_start) / _day_start * 100, 2)
        except Exception as _e:
...
def get_trading_status():
    ...
    _max_single_sym = ""
    _all_positions = _qmt_positions if _qmt_positions else state.positions  # QMT优先
...
def execute_trade(action, code, price=None, quantity=None):
    """执行交易 — 根据 trading_channel 自动路由 (E349: 加风控检查)"""
    # 🔒 实盘双锁
    if CONFIG.get("trading_channel") == "qmt" and not CONFIG.get("real_confirmed", False):
        return {"success": False, "error": "实盘未确认(real_confirmed=false)"}
    # 熔断检查
    if state.risk.get("circuit_breaker", False):
        return {"success": False, "error": "熔断已触发,禁止交易"}
    if state.risk.get("daily_loss", 0) <= CONFIG.get("max_daily_loss", -0.05) * 100:
        return {"success": False, "error": f"日亏损已达上限({state.risk['daily_loss']}%),禁止新开仓"}
    if action == "buy" and state.daily_trade_count >= CONFIG.get("max_daily_trades", 5):
        return {"success": False, "error": f"日内交易次数已达上限({CONFIG.get('max_daily_trades',5)})"}
    _use_qmt = CONFIG.get("trading_channel") == "qmt" and _qmt_available
    if _use_qmt:
        if action == "buy":
...
def _get_auto_status():
    """真实自动交易状态: CONFIG + KS ai_auto_enabled 双重检查"""
    try:
        import json as _js, os as _os
        _pp = r"D:\quant_web\data\auto_trade_plan.json"
        if _os.path.exists(_pp):
            _ai = _js.load(open(_pp, "r", encoding="utf-8")).get("global_limits", {}).get("ai_auto_enabled")
            if _ai is False:
                return False
    except: pass
...
def toggle_auto_trade(enabled=None):
    ...
    return {"auto_trade_enabled": _get_auto_status()}
