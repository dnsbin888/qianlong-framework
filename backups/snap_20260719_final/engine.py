...
class RuleEngine:
    ...
    def check_daily_drop(self, position: dict) -> dict | None:
        """D3: 单票日盈亏 — 昨收对比, 日跌超限强制减仓。

        与 stop_loss 的区别:
          stop_loss: 当前价 vs 持仓成本 (累计盈亏)
          daily_drop: 当前价 vs 昨日收盘价 (单日盈亏)

        Args:
            position: {symbol, avg_cost, last_price, prev_close, qty, ...}
        ...
        prev_close = float(position.get("prev_close", 0))
        ...
        if daily_chg <= -0.05:
            return {"action": "sell", "reason": f"单票日跌超5%({daily_chg*100:.1f}%)→清仓", "qty": qty}
        if daily_chg <= -0.03:
            sell_qty = max(100, (qty // 2) // 100 * 100)
            return {"action": "sell", "reason": f"单票日跌超3%({daily_chg*100:.1f}%)→卖半仓", "qty": sell_qty}
        return None
    ...
    def check_stop_loss(self, position: dict) -> dict | None:
        ...
        # E371: 止损阈值从 trade_config_master 读取
        try:
            _mcfg = __import__('json').load(open(r"D:\quant_framework\trade_config_master.json", encoding="utf-8"))
            _hard = _mcfg.get("stop_loss", {}).get("hard", -0.055)
            _soft = _mcfg.get("stop_loss", {}).get("soft", -0.03)
        except Exception:
            _hard, _soft = -0.055, -0.03  # 兜底仅在最极端情况
        if pnl_pct <= _hard:
    ...
    def check_concentration(self, positions: dict, new_symbol: str, new_value: float,
                            total_asset: float = None) -> tuple[bool, str]:
        """R1-5: 检查集中度 (单票/总资产)。
        ...
        if total_asset is None or total_asset <= 0:
            ...
            total_asset = total_existing + new_value if (total_existing + new_value) > 0 else 1

        # 同票已有持仓+新买 = 总集中度
        existing_same = 0.0
        for sym, p in (positions.items() if isinstance(positions, dict) else {}):
            if sym == new_symbol:
                existing_same = float(p.get("market_value", 0) or
                    float(p.get("last_price", p.get("avg_cost", 0))) * int(p.get("qty", 0)))
                break

        new_pct = (existing_same + new_value) / total_asset * 100
        if new_pct > 50:
